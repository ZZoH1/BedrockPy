...
assert.ok(extensionSource.includes("message.type === 'exportMcstructure'"));
assert.ok(extensionSource.includes("message.type === 'requestMcstructureExport'"));
assert.ok(extensionSource.includes("type: 'mcstructureExportApproved'"));
assert.ok(extensionSource.includes('id="export-mcstructure"'));
assert.ok(extensionSource.includes('다른 이름으로 저장'));
assert.ok(extensionSource.includes('const MCSTRUCTURE_MAX_SIZE = { x: 64, y: 384, z: 64 }'));
assert.ok(extensionSource.includes('async function createMcstructure'));
assert.ok(extensionSource.includes('function mcstructureBounds(data)'));
assert.ok(extensionSource.includes('min: { x: 0, y: 0, z: 0 }'));
assert.ok(extensionSource.includes("nbtList('block_indices', 9"));
assert.ok(extensionSource.includes("nbtList('block_palette', 10"));
assert.ok(extensionSource.includes('!Array.isArray(data.layers)'));
assert.ok(extensionSource.includes("name: '레이어 1', visible: true, blocks: []"));
assert.ok(extensionSource.includes("message.type === 'confirmLayerDelete'"));
assert.ok(extensionSource.includes("type: 'layerDeleteConfirmed'"));
...
assert.ok(extensionSource.includes('id="layer-list"'));
assert.ok(extensionSource.includes('id="add-layer"'));
assert.ok(!extensionSource.includes('id="move-layer-up"'));
assert.ok(!extensionSource.includes('id="move-layer-down"'));
...
assert.ok(voxelSource.includes('type: "exportMcstructure"'));
assert.ok(voxelSource.includes('type: "requestMcstructureExport"'));
assert.ok(voxelSource.includes('message.type === "mcstructureExportApproved"'));
...
assert.ok(voxelSource.includes('gridStep * 224 * (4 ** level)'));
...
assert.ok(voxelSource.includes('function unlockOriginalTransformPlacement()'));
assert.ok(voxelSource.includes('(next === "moveSelection" || next === "paste") && previousTool !== next'));
assert.ok(voxelSource.includes('if (unlockOriginalTransformPlacement()) return;'));
assert.ok(voxelSource.includes('if (pendingPlacement && next !== tool) cancelPendingPlacement();'));
assert.ok(!voxelSource.includes('if (pendingPlacement && next !== tool) commitPendingPlacement();'));
...
assert.ok(voxelSource.includes('class LayerBlockMap extends Map'));
assert.ok(voxelSource.includes('function createLazyComposedLayerMap()'));
assert.ok(voxelSource.includes('function rebuildChangedLayerChunks('));
assert.ok(voxelSource.includes('function affectedLayerChunkKeys('));
assert.ok(voxelSource.includes('if (equalComposedChunks(before, after)) continue'));
assert.ok(voxelSource.includes('const occupancy = new Uint8Array(occupancySide ** 3)'));
assert.ok(voxelSource.includes('const faceMask = new Uint8Array(renderChunkSize ** 3)'));
assert.ok(voxelSource.includes('faceMask[planeOffset + (startU + du) * 16 + startV + dv] = 0'));
assert.ok(voxelSource.includes('previous.material === material'));
assert.ok(!voxelSource.includes('planes.get(plane).add(`${cell[face.u]},${cell[face.v]}`)'));
assert.ok(voxelSource.includes('function appendDeferredBrushCenter('));
const placementPlanner = voxelSource.match(/function planPlacementBrush[\s\S]*?\n}\n\nfunction planDeletionBrush/)?.[0] || '';
const deletionPlanner = voxelSource.match(/function planDeletionBrush[\s\S]*?\n}\n\nfunction planSculptCenter/)?.[0] || '';
assert.ok(placementPlanner.includes('appendDeferredBrushCenter('));
assert.ok(deletionPlanner.includes('appendDeferredBrushCenter('));
assert.ok(!placementPlanner.includes('blocks.has('));
assert.ok(!deletionPlanner.includes('blocks.has('));
assert.ok(voxelSource.includes('if (!structureDataLoading && !deferredBrushCommitActive)'));
assert.ok(voxelSource.includes('let selectedType = null'));
assert.ok(voxelSource.includes('if (!fillType) return'));
assert.ok(!voxelSource.includes('let selectedType = "stone"'));
assert.ok(voxelSource.includes('const deferredBrushJobs = []'));
assert.ok(voxelSource.includes('function enqueueDeferredBrushPath('));
assert.ok(voxelSource.includes('while (deferredBrushJobs.length)'));
assert.ok(voxelSource.includes('captureDeferredBrushConfig('));
assert.ok(!voxelSource.includes('if (deferredBrushCommitActive) {\n    beginCameraDrag(event, "cameraDuringCommit")'));
assert.ok(voxelSource.includes('liveEditPreviewCells.set(position, { erase, color: previewColor })'));
assert.ok(voxelSource.includes('geometry.setAttribute("color", new THREE.Float32BufferAttribute(colors, 3))'));
assert.ok(voxelSource.includes('vertexColors: true'));
assert.ok(voxelSource.includes('function scheduleSelectionSurfacePreview('));
assert.ok(voxelSource.includes('let selectionSurfaceBuildToken = 0'));
assert.ok(voxelSource.includes('async function keepOnlyInstalledSelection()'));
assert.ok(voxelSource.includes('await nextUiFrame()'));
const pasteApplyBranch = voxelSource.match(/if \(tool === "paste"\) \{[\s\S]*?\n  \}/)?.[0] || '';
const pastePreviewBranch = voxelSource.match(/if \(tool === "paste"\) return transformedPreviewPlacement[\s\S]*?\n    \);/)?.[0] || '';
assert.ok(pasteApplyBranch.includes('putGenerated(x, y, z, type, false)'));
assert.ok(!pasteApplyBranch.includes('brushAllowed('));
assert.ok(!pastePreviewBranch.includes('brushAllowed('));
assert.ok(extensionSource.includes('id="selection-block-preview"'));
assert.ok(voxelSource.includes('selectionBlockPreview: Boolean('));
assert.ok(voxelSource.includes('function resetSelectionSurfacePreviewData()'));
assert.ok(voxelSource.includes('if (!document.getElementById("selection-block-preview")?.checked)'));
assert.ok(voxelSource.includes('if (next === "moveSelection" && previousTool !== next)'));
assert.ok(voxelSource.includes('origin = transformPlacementOriginalOrigin(next)'));
assert.ok(voxelSource.includes('chunks: layer.blocks.serializedChunks()'));
assert.ok(voxelSource.includes('lazyComposedLayerChunks'));
assert.ok(voxelSource.includes('function composeVisibleLayerEntries()'));
assert.ok(voxelSource.includes('function rebuildComposedLayers('));
assert.ok(voxelSource.includes('layers: layers.map(layer => ({'));
assert.ok(voxelSource.includes('activeLayerId'));
assert.ok(voxelSource.includes(':not(.project-section):not(.layer-section)'));
assert.ok(voxelSource.includes('function reorderLayerByDrop('));
assert.ok(voxelSource.includes('while (usedNumbers.has(layerNumber)) layerNumber++'));
assert.ok(voxelSource.includes('name: "레이어 1"'));
assert.ok(!voxelSource.includes('name: "기본 레이어"'));
assert.ok(voxelSource.includes('item.addEventListener("dragstart"'));
assert.ok(voxelSource.includes('className = "layer-name-input"'));
assert.ok(voxelSource.includes('kind: "layer-delete"'));
assert.ok(voxelSource.includes('kind: "layer-add"'));
assert.ok(voxelSource.includes('kind: "layer-rename"'));
assert.ok(voxelSource.includes('kind: "layer-visibility"'));
assert.ok(voxelSource.includes('kind: "layer-reorder"'));
assert.ok(voxelSource.includes('kind: "transform"'));
assert.ok(voxelSource.includes('function restoreTransformPlacementState('));
assert.ok(voxelSource.includes('storeTransformPlacementUndo(beforeTransform'));
...
assert.ok(voxelSource.includes('const showProgress = totalBlocks >= 5000'));
...
assert.ok(voxelSource.includes('functionName: defaultFunctionNameForFile(currentFile)'));
...
assert.ok(!extensionSource.includes('id="function-name"'));
assert.ok(!extensionSource.includes('data-utility="code"'));
assert.ok(!extensionSource.includes('data-utility-label="코드·정보"'));
assert.ok(extensionSource.includes('data-utility-label="정보"'));
assert.ok(!voxelSource.includes('document.getElementById("function-name")'));
...
for (const feature of [
  ...
  'enqueueDeferredBrushPath', 'processDeferredBrushJobs',
...
assert.ok(extensionSource.includes('id="paste-area-preview"'));
assert.ok(extensionSource.includes('id="move-selection-area-preview"'));
assert.ok(extensionSource.includes('id="cursor-task-progress"'));
assert.ok(extensionSource.includes('class="brand-mark" src="${logoUri}"'));
assert.ok(!extensionSource.includes('id="cursor-task-label"'));
assert.ok(voxelCss.includes('transform: translate(-50%, 14px)'));
assert.ok(voxelSource.includes('function updateCursorTaskProgress'));
assert.ok(voxelSource.includes('updateCursorTaskProgress("brush"'));
assert.ok(voxelSource.includes('let deferredBrushProgressTotal = 0'));
assert.ok(voxelSource.includes('deferredBrushProgressTotal += copiedCenters.length'));
assert.ok(voxelSource.includes('deferredBrushProgressProcessed / Math.max(1, deferredBrushProgressTotal)'));
assert.ok(voxelSource.includes('state.progressKind = tool'));
assert.ok(voxelSource.includes('async function runReplaceOperation'));
assert.ok(voxelSource.includes('updateCursorTaskProgress("replace"'));
assert.ok(voxelSource.includes('void runReplaceOperation(cell'));
assert.ok(!voxelSource.includes('updateCursorTaskProgress("overlay"'));
assert.ok(extensionSource.includes("message.type === 'requestBpyExport'"));
assert.ok(extensionSource.includes("message.type === 'requestMcworldExport'"));
assert.ok(voxelSource.includes('message.type === "bpyExportApproved"'));
assert.ok(voxelSource.includes('message.type === "mcworldExportApproved"'));
assert.ok(voxelSource.includes('function transformAreaPreviewEnabled'));
assert.ok(voxelSource.includes('function transformOutlineDimensions'));
assert.ok(voxelSource.includes('pasteAreaPreview: Boolean('));
assert.ok(voxelSource.includes('moveSelectionAreaPreview: Boolean('));
...
assert.ok(extensionSource.includes('id="show-map-boundary" type="checkbox" checked'));
assert.ok(extensionSource.includes('id="show-floor-grid" type="checkbox" checked'));
assert.ok(voxelCss.includes('.workspace-guide-toggle input[type="checkbox"]'));
assert.ok(voxelSource.includes('function updateWorkspaceGuideVisibility('));
assert.ok(voxelSource.includes('function handlePlayJumpTap()'));
assert.ok(voxelSource.includes('if (previewOnly) {\n    selectionSurfaceBuildToken++;\n    clearSelectionSurfaceMesh();'));
assert.ok(voxelSource.includes('selectionFill.visible = false;\n    return;'));
assert.ok(voxelSource.includes('canvas.addEventListener("dblclick", event => {'));
assert.ok(voxelSource.includes('function selectConnectedFromMoveDoubleClick('));
assert.ok(voxelSource.includes('const isDoubleClick = Boolean(previous && now - previous.time <= 350'));
assert.ok(voxelSource.includes('Math.hypot(event.clientX - previous.x, event.clientY - previous.y) <= 8'));
assert.ok(voxelSource.includes('if (playMode || tool !== "move" || event.button !== 0) return'));
assert.ok(voxelSource.includes('const preservePlacementMode = pending.tool === "paste" || pending.tool === "moveSelection"'));
assert.ok(voxelSource.includes('locked: continueLocked'));
assert.ok(voxelSource.includes('transformPlacementOriginalOrigin("moveSelection") || cloneCell(appliedOrigin)'));
assert.ok(voxelSource.includes('const sourceType = layerBlocks.get(key(start.x, start.y, start.z))'));
assert.ok(voxelSource.includes('const relevantChunks = [...layerBlocks.chunkCounts().keys()]'));
assert.ok(voxelSource.includes('activeLayerId, selectionMask.size, firstMaskPosition'));
assert.ok(voxelSource.includes('layerAwareSelection ? activeLayer()?.blocks : blocks'));
assert.ok(voxelSource.includes('tool === "move" || tool === "moveSelection"'));
assert.ok(voxelSource.includes('function copySelection(cut = false, preserveTransformState = false)'));
assert.ok(voxelSource.includes('copySelection(false, tool === "moveSelection")'));
assert.ok(voxelSource.includes('const sourceType = layerBlocks.get(key(start.x, start.y, start.z))'));
assert.ok(voxelSource.includes('if (layerBlocks.get(position) === sourceType) targets.push(position)'));
assert.ok(!voxelSource.includes('tool === "replace" || tool === "sculpt"'));
assert.ok(!voxelSource.includes('tool === "replace" || tool === "selectBox"'));
assert.ok(voxelSource.includes('function columnTop(x, z, blockSource = activeLayer()?.blocks)'));
assert.ok(voxelSource.includes('const blockSource = options.blockSource || activeLayer()?.blocks'));
assert.ok(voxelSource.includes('thermalErodeColumns(columns, strength * 2, 0, workspaceSize.y - 1, layerBlocks)'));
assert.ok(voxelSource.includes('const occupiedInActiveLayer = activeLayer()?.blocks.has(key(x, y, z)) || false'));
assert.ok(voxelSource.includes('depthWrite: false, depthTest: false, roughness: 0.76'));
assert.ok(voxelSource.includes('return true;\n}\n\nfunction cellInSelection'));
assert.ok(voxelSource.includes('playerFlying = !playerFlying'));
assert.ok(voxelSource.includes('const verticalAmount = (pressedKeys.has("space") ? 1 : 0) - (pressedKeys.has("shift") ? 1 : 0)'));
assert.ok(voxelSource.includes('showMapBoundary: Boolean(document.getElementById("show-map-boundary")?.checked)'));
assert.ok(voxelSource.includes('showFloorGrid: Boolean(document.getElementById("show-floor-grid")?.checked)'));
...
assert.ok(voxelSource.includes('const outlineOnlyPreview = outlineOnlyTransform || (placementPreview && previewCellCount > 1000)'));
...
assert.ok(voxelSource.includes('tool === "sculpt" || isDeferredShapeTool()'));
assert.ok(voxelSource.includes('curve.getSpacedPoints(steps)'));
assert.ok(voxelSource.includes('const bridgeSteps = Math.max(Math.abs(dx), Math.abs(dy), Math.abs(dz))'));
