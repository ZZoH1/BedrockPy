...
assert.ok(extensionSource.includes('structurePanel.reveal(structurePanel.viewColumn, false)'));
assert.ok(extensionSource.includes("type: 'editorMovedToNewWindow'"));
...
assert.ok(/new-window-editor"\)\?\.addEventListener\("click", event => \{[\s\S]*?event\.currentTarget\.disabled = true/.test(voxelSource));
assert.ok(/message\.type === "editorMovedToNewWindow"[\s\S]*?button\.hidden = true/.test(voxelSource));
assert.ok(voxelCss.includes('.new-window-editor-button[hidden] { display: none; }'));
...
assert.ok(extensionSource.includes('https://github.com/Mojang/bedrock-samples.git'));
