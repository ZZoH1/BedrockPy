...
- 박스 선택의 마우스 포인터와 드래그 시작·끝 좌표가 모든 보이는 레이어 표면을 대상으로 동작
- 박스 안의 블록 미리보기와 설치된 블록 필터는 현재 활성 레이어만 유지

## 0.157.0

- 조형 도구의 포인터 외곽선과 브러시 위치는 모든 보이는 레이어를 기준으로 유지
- 올리기·내리기·평탄화·부드럽게·중력 평탄화의 높이 및 블록 종류 계산을 활성 레이어 기준으로 변경
- 객체 다듬기·객체 연결의 주변 블록 탐색에서 다른 레이어를 제외
- 선택 영역 조형도 활성 레이어 블록만 읽고 변경하도록 수정

## 0.156.8

- 교체 도구의 마우스 포인터와 외곽선이 현재 활성 레이어 블록만 찾도록 변경
- 일반 교체와 연결 교체의 원본 종류 판정 및 대상 탐색을 활성 레이어로 제한
- 다른 레이어가 앞을 가려도 통과해 현재 레이어 블록을 교체하고 다른 레이어 데이터는 유지

## 0.156.7

- 선택 이동 도구에서 `Cmd/Ctrl+C`로 현재 활성 레이어의 선택 블록 복사 지원
- 복사 시 선택 이동 기즈모·위치·회전·배율·도구 상태를 변경하지 않도록 수정
- 복사 단축키 이벤트가 브라우저 텍스트 복사나 다른 단축키 처리로 이어지지 않도록 단독 종료

## 0.156.6

- 선택 이동 도구의 일반 마우스 호버 레이캐스트도 현재 활성 레이어만 대상으로 변경
- 다른 레이어 블록에 선택 외곽선이 표시되거나 포인터 추적 기준으로 잡히던 문제 수정

## 0.156.5

- VS Code Webview에서 `PointerEvent.detail`이 증가하지 않아 선택 이동 더블클릭이 감지되지 않던 문제 수정
- 350ms 이내 8px 범위의 연속 클릭을 직접 판별하는 방식으로 변경
- 기즈모 핸들 클릭과 도구 전환은 더블클릭 후보에서 제외

## 0.156.4

- 선택 이동에서 더블클릭의 두 번째 누름을 기존 좌클릭·기즈모 처리보다 먼저 판별
- 더블클릭 연결 선택과 기존 좌클릭 위치 고정 동작이 서로 중복 실행되지 않도록 이벤트 분리
- 브라우저 `dblclick` 이벤트의 연결 선택 중복 실행 방지

## 0.156.3

- 선택 이동 도구에서도 활성 레이어 블록 더블클릭 연결 선택 지원
- 선택 이동 중 일반 더블클릭은 선택 교체, `Shift + 더블클릭`은 기존 선택에 추가
- 새 연결 선택을 만든 뒤 해당 선택의 원위치 기즈모를 즉시 다시 생성

## 0.156.2

- 이동 도구에서 `Shift + 더블클릭`하면 기존 선택을 유지하면서 연결된 활성 레이어 블록을 추가 선택
- 일반 더블클릭은 기존 선택을 교체하고, 두 방식 모두 선택 이동 도구로 자동 전환

## 0.156.1

- 배치의 공기에만·블록에만 조건을 합성 화면이 아닌 현재 활성 레이어 기준으로 판정
- 다른 레이어가 같은 좌표를 덮고 있어도 현재 레이어에 블록을 정상 배치하도록 수정
- 이동·박스·브러시 선택 포인팅이 다른 레이어를 통과해 활성 레이어의 가려진 블록을 찾도록 변경
- 선택 블록 강조와 단순 표면 미리보기를 다른 레이어 뒤에서도 확인할 수 있게 표시

## 0.156.0

- 연결 선택과 이동 도구 더블클릭 선택이 현재 활성 레이어의 블록만 탐색하도록 변경
- 브러시 선택 및 `설치된 블록 칸만 선택` 필터가 활성 레이어 블록만 선택하도록 변경
- 선택 이동·복사·회전·확대축소 및 선택 미리보기에서 다른 레이어의 합성 블록이 섞이지 않도록 수정
- 전체 선택도 현재 활성 레이어의 블록 범위만 대상으로 제한

## 0.155.1

- 붙여넣기·선택 이동을 기즈모 상태에서 우클릭 적용하면 다음 작업도 기즈모 상태로 유지
- 커서 추적 상태에서 우클릭 적용하면 다음 작업도 기즈모 없이 마우스 포인터를 계속 추적
- 선택 이동 적용 후에는 이동된 새 선택 영역의 원점을 다음 기즈모 위치로 사용

## 0.155.0

- 이동 도구에서 블록을 좌클릭으로 더블클릭하면 면으로 연결된 모든 비공기 블록을 자동 선택
- 연결 선택 직후 원래 위치의 기즈모가 열린 선택 이동 도구로 자동 전환
- 공기나 작업공간 바닥을 더블클릭한 경우에는 도구를 전환하지 않도록 처리

## 0.154.1

- 박스 선택 드래그 중에는 선택 외곽선만 실시간으로 표시하도록 변경
- 선택 내부 블록 및 반투명 채우기 미리보기는 마우스를 놓은 뒤 한 번만 생성
- 새 박스 선택을 시작할 때 이전 선택의 비동기 표면 미리보기 작업을 즉시 취소

## 0.154.0

- Play Mode에서 `Space`를 빠르게 두 번 눌러 비행 모드를 켜고 끄는 기능 추가
- 비행 중 `Space`로 상승하고 `Shift`로 하강하며 중력과 착지 계산을 비활성화
- Play Mode에 다시 진입하면 비행 상태가 기본적으로 해제되도록 처리

## 0.153.2

- 환경 설정의 맵 외곽선·바닥 격자 체크박스가 입력 필드 너비로 크게 표시되던 문제 수정
- 두 체크박스를 13px 크기의 간결한 설정 항목으로 정돈

## 0.153.1

- 환경 설정에 `맵 외곽선 표시`와 `바닥 격자 표시` 체크박스 추가
- 두 표시 설정을 구조물별 `.bpstructure` 설정에 저장하고 다시 열 때 복원
- 바닥 격자를 숨긴 동안 적응형 격자 갱신을 건너뛰도록 최적화

## 0.153.0

- 제어점이 많거나 전체 길이가 긴 곡선을 거리 기준으로 균등 샘플링하도록 변경
- 반올림된 곡선 샘플 사이를 복셀 선으로 추가 보간하여 중간이 끊기는 현상 수정

## 0.152.9

- 구·빈 구·원·원판·원기둥·산을 연속 배치할 때 중심 경로 사이의 빠진 좌표를 보간
- 마우스를 빠르게 움직여도 배치 도형 사이가 끊기지 않고 연속적으로 이어지도록 개선

## 0.152.8

- 카메라 주변에서 1×1 바닥 격자가 유지되는 반경을 160블록에서 224블록으로 확대
- 바깥쪽의 4×4, 16×16 단계 전환 구조와 로컬 좌표 정밀도 보정은 그대로 유지

## 0.152.7

- 구·빈 구·원·원판·원기둥·산 배치 작업을 지연 작업 대기열과 포인터 진행 바에 연결
- 여러 도형을 연속 배치하면 기존 배치·삭제·조형과 동일하게 전체 대기 작업량으로 진행률 계산
- 일반 교체와 연결 교체를 프레임 단위 비동기 탐색·적용으로 변경하고 포인터 진행률 표시
- 큰 선택 영역 교체와 큰 연결 영역 탐색 중에도 화면 갱신 및 진행도 확인 가능

## 0.152.6

- 중앙 진행 화면이 표시되는 저장·내보내기 작업은 포인터 진행 바에서 제외
- `.bpy`, `.mcstructure`, `.mcworld` 모두 저장 위치를 먼저 선택한 뒤 데이터 계산 시작
- `.bpy`는 저장 승인 후에만 블록 압축과 명령 코드 생성 실행
- `.mcworld`는 저장 승인 후에만 지연 청크 로딩과 전체 블록 직렬화 실행

## 0.152.5

- `.mcstructure` 내보내기에서 블록·청크를 수집하기 전에 작업공간 크기부터 검사
- 크기 제한을 통과하고 저장 위치까지 선택한 경우에만 전체 블록 데이터를 메모리에 구성
- 제한 초과 또는 저장 취소 시 지연 청크 로딩과 블록 직렬화를 전혀 실행하지 않도록 변경

## 0.152.4

- 3D 편집기의 `코드·정보` 보조 메뉴 이름을 `정보`로 변경
- 보조 메뉴의 `코드 생성` 섹션과 함수 이름 입력란 제거
- `.bpy` 코드 생성 함수 이름을 현재 `.bpstructure` 또는 `.mcstructure` 파일명에서 자동 생성
- `.mcstructure`와 `.mcworld` 내보내기 기본 이름도 현재 구조물 파일명을 자동 사용
- 함수 이름을 `.bpstructure` 편집 설정에 별도로 저장하지 않도록 변경

## 0.152.3

- 블록 작업 진행 바를 개별 작업이 아닌 현재 실행 및 대기 중인 전체 작업량 기준으로 계산
- 이전 작업 처리 중 새 배치·삭제·조형 작업을 추가하면 전체 작업량과 남은 진행률을 즉시 재조정
- 작업별로 진행 바를 다시 시작하지 않고 모든 대기 작업이 완료된 뒤에만 숨김

## 0.152.2

- 3D 편집기 상단 `B` 표식을 BedrockPy 공식 아이콘 이미지로 교체
- 포인터 진행 표시에서 작업명과 네모난 배경을 제거
- 마우스 바로 아래 중앙에 외곽선만 있는 얇은 알약형 진행 바로 재디자인
- 진행률에 따라 알약 내부가 왼쪽부터 채워지도록 표시

## 0.152.1

- `.mcstructure` 내보내기 범위를 설치된 블록의 외곽이 아닌 현재 작업공간 전체 크기로 변경
- 작업공간 안의 빈 여백도 그대로 구조물 범위와 공기 인덱스에 포함
- 실제 블록 크기가 아닌 작업공간 크기를 기준으로 Bedrock `64×384×64` 제한 검사

## 0.152.0

- 상단 `다른 이름으로` 버튼 이름을 `다른 이름으로 저장`으로 변경
- 3D 구조물을 Bedrock little-endian NBT `.mcstructure` 파일로 직접 내보내는 기능 추가
- 보이는 레이어의 실제 블록 외곽에 맞춰 빈 여백을 제거하고 팔레트 및 블록 인덱스 생성
- Bedrock 최대 크기 `64×384×64`를 넘으면 저장 전에 초과 크기와 축을 안내하고 내보내기 차단

## 0.151.9

- 실행 중인 작업이 있을 때 마우스 포인터 아래에 작은 작업명과 진행 바 표시
- 블록 배치·삭제·조형의 지연 적용 진행률을 처리된 경로에 맞춰 갱신
- 선택 영역 작업, 저장, 가져오기와 내보내기 등 기존 진행 작업도 포인터 진행 바에 연동
- 작업 완료 시 포인터 진행 표시를 숨기고 동시에 남은 작업이 있으면 해당 진행률 유지

## 0.151.8

- 선택 이동과 붙여넣기 패널에 각각 `이동 영역 미리보기` 체크박스 추가
- 옵션을 끄면 이동할 블록의 상세·단순화 표면 생성을 생략하고 전체 외곽선과 기즈모만 표시
- 두 미리보기 설정을 `.bpstructure` 파일별 편집기 설정에 저장

## 0.151.7

- 붙여넣기·선택 이동·특수 배치의 기즈모 조작 중 다른 도구를 선택해도 작업이 자동 적용되지 않도록 수정
- 미확정 기즈모 작업은 도구 전환 시 취소하고 우클릭 또는 명시적인 확정 입력에서만 실제 블록에 반영

## 0.151.6

- 붙여넣기도 복사한 원래 위치에 고정된 기즈모 상태로 시작
- 선택 이동과 붙여넣기에서 `Esc`를 누르면 취소 대신 커서를 따라가는 위치 지정 단계로 전환
- 위치 지정 단계에서 좌클릭하면 해당 위치에 기즈모를 다시 고정

## 0.151.5

- 선택 이동 도구 진입 시 선택 영역의 원래 위치에 미리보기를 즉시 고정
- 별도의 최초 위치 지정 없이 이동·회전·크기·축 늘이기 기즈모를 기본 표시
- 선택 이동 도움말을 원위치 기즈모 조작 방식에 맞게 수정

## 0.151.4

- 선택 영역 패널에 `선택 영역 블록 미리보기` 체크박스 추가
- 옵션을 끄면 보라색 외곽선만 유지하고 내부 블록 수집, 청크 디코딩, 표면 메쉬 생성을 모두 생략
- 실행 중 옵션을 끄면 진행 중인 선택 표면 계산을 취소하고 관련 캐시와 메쉬 해제
- 선택한 값은 `.bpstructure` 편집기 설정에 저장

## 0.151.3

- `선택 안에서만` 옵션이 붙여넣기 미리보기와 실제 붙여넣기를 선택 영역으로 제한하던 문제 수정
- 붙여넣기는 선택 영역 제한과 무관하게 전체 클립보드 구조물을 배치하고, 공기 붙여넣기 설정만 별도로 적용

## 0.151.2

- 큰 박스 선택을 확정할 때 구조물의 모든 블록과 지연 청크를 동기 순회하던 문제 수정
- 선택 외곽은 즉시 표시하고 보라색 블록 표면 미리보기는 선택 영역과 겹치는 청크만 5ms 단위로 비동기 수집
- `설치된 블록 칸만 선택`도 선택 부피 전체가 아니라 겹치는 청크의 실제 블록만 비동기 검사
- 새 선택을 시작하면 이전 선택의 표면 계산을 취소하도록 처리

## 0.151.1

- 삭제 작업 진행 중 이동 도구나 선택 블록으로 전환하면 경로 미리보기 색상이 바뀌던 문제 수정
- 미리보기 점마다 생성 당시의 삭제·조형·배치 색상을 저장하고 정점 색상으로 렌더링
- 서로 다른 종류의 작업이 큐에 있어도 각 경로의 원래 색상 유지

## 0.151.0

- 대형 배치·삭제·조형 경로가 적용되는 동안에도 새 작업 경로를 추가할 수 있는 직렬 작업 큐 구현
- 새로 그린 경로는 기존 작업 다음에 순서대로 실행되고 각 작업을 별도의 Undo 단계로 기록
- 작업마다 블록 종류, 브러시 크기·모양, 설치 조건, 조형 모드·강도를 캡처해 큐 처리 중 설정 혼합 방지
- 큐 전체가 끝난 후 누적된 Dirty 청크를 한 번만 렌더링하도록 유지

## 0.150.3

- 조형 작업에서 주변 블록 종류를 찾지 못했을 때 `stone`을 기본 생성하던 동작 제거
- 빈 기둥 높이기와 객체 다듬기·연결 시 참고할 블록이 없으면 해당 채우기를 안전하게 건너뜀

## 0.150.2

- 대형 연속 배치·삭제·조형 적용 중 새 청크가 감지되어 중간 메쉬가 생성되던 문제 수정
- 마우스를 놓은 뒤 경로 처리 중에는 청크 스트리밍과 메쉬 재생성을 잠그고 완료 후 누적 Dirty 청크를 한 번만 렌더 큐에 반영
- 적용 중 카메라 이동과 기존 구조물 화면 렌더링은 유지

## 0.150.1

- 큰 브러시 연속 배치·삭제 중 브러시 부피 전체와 기존 블록을 매번 검사하던 문제 제거
- 드래그 중에는 중복 제거된 중심 경로와 고정 개수 미리보기 점만 기록
- 선택 영역, 공기에만/블록에만 설치, 브러시 모양 계산은 마우스를 놓은 뒤 프레임 분할 적용 단계에서 처리
- 연속 도형 배치도 드래그 중 전체 도형을 생성하지 않고 중심 경로만 저장

## 0.150.0

- 청크 메쉬 생성 시 블록마다 좌표 문자열로 이웃을 조회하던 과정을 18×18×18 `Uint8Array` 점유 배열로 교체
- Greedy Meshing의 `Map`/`Set` 문자열 좌표를 16×16×16 숫자형 면 마스크로 교체해 메모리 할당과 GC 비용 감소
- 동일 재질이 연속되는 면 그룹을 합쳐 단색 렌더링과 동일 텍스처 블록의 Draw Call 감소
- 레이어 변경 시 실제 경계 블록이 달라진 방향의 이웃 청크만 Dirty 처리

## 0.149.1

- 레이어 표시·삭제·추가·순서 변경 및 Undo/Redo 시 전체 합성 메쉬를 폐기하지 않도록 개선
- 변경 전후 합성 결과를 영향받은 청크별로 비교해 실제 최종 블록이 달라진 청크만 재생성
- 동일한 블록에 가려져 결과가 변하지 않은 청크는 렌더 갱신에서 제외

## 0.149.0

- 레이어별 블록을 16×16×16 청크와 Base64 페이로드로 저장하도록 `.bpstructure` 형식 개선
- 구조물 로딩 시 모든 레이어 블록을 즉시 Map으로 변환하지 않고 카메라 주변 청크만 지연 디코딩
- 보이는 레이어의 합성 결과도 요청된 렌더 청크 단위로 생성하고 멀어진 깨끗한 청크는 메모리에서 해제
- 0.148.x의 레이어 `blocks` 배열 파일은 불러올 때 새 청크 구조로 자동 변환

## 0.148.3

- 레이어 추가·삭제·이름 변경·표시 전환·드래그 순서 변경을 각각 Undo/Redo 가능하도록 통합
- 레이어 합성 메쉬 갱신 시 기존 Undo/Redo 기록이 사라지던 문제 수정

## 0.148.2

- 이전 `chunks` 기반 단일 레이어 `.bpstructure` 형식 지원 제거
- 새 구조물과 새 프로젝트 파일을 처음부터 `layers` 형식과 `레이어 1`로 생성
- 외부 Minecraft `.mcstructure` 가져오기는 블록을 `레이어 1`에 배치하도록 유지

## 0.148.1

- 새 구조물과 기존 단일 레이어 구조물의 최초 레이어 이름을 `기본 레이어`에서 `레이어 1`로 변경
- 이후 추가 레이어가 `레이어 2`부터 자연스럽게 이어지도록 이름 규칙 통일

## 0.148.0

- 붙여넣기·선택 이동 미리보기의 화살표 이동, 확대·축소, 늘이기, 회전 조작을 Undo/Redo 기록에 추가
- 블록이 존재하는 레이어를 삭제할 때 VS Code 확인창 표시
- 삭제한 레이어의 이름·순서·가시성·전체 블록을 Cmd/Ctrl+Z로 복원
- 블록 편집·미리보기 변형·레이어 삭제를 하나의 작업별 Undo 스택에서 한 번에 하나씩 처리
- 텍스트 입력 중에는 입력값 Undo만, 3D 화면에서는 구조물 Undo만 실행하도록 분리

## 0.147.3

- 삭제한 레이어 번호를 새 레이어 생성 시 재사용하지 않던 문제 수정
- 현재 존재하는 레이어 이름에서 비어 있는 가장 작은 번호를 자동 선택

## 0.147.2

- 레이어 형식으로 저장한 `.bpstructure`를 기존 `chunks` 전용 검사기가 거부하던 문제 수정
- `layers` 또는 기존 `chunks` 형식을 모두 정상적인 구조물 파일로 인식
- 이미 0.147.x에서 저장한 레이어 구조물도 수정 없이 다시 열 수 있도록 호환

## 0.147.1

- 웹뷰 기본 대화상자에 의존하던 레이어 이름 변경과 삭제가 작동하지 않던 문제 수정
- 레이어 이름을 목록 안에서 직접 편집하도록 변경
- 위·아래 순서 버튼을 제거하고 레이어 행 드래그로 순서를 변경하도록 개선
- 드래그 삽입 위치와 이동 중인 레이어를 시각적으로 표시

## 0.147.0

- 3D 구조물 프로젝트 패널 아래에 레이어 패널 추가
- 레이어 추가·삭제·이름 변경·위아래 순서 변경 지원
- 레이어별 표시 여부 토글 및 활성 편집 레이어 선택 지원
- 같은 좌표의 블록은 가장 위에 있는 보이는 레이어가 덮도록 합성
- 기존 `.bpstructure`를 기본 레이어 하나로 자동 호환
